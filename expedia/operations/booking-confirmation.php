                ...
                foreach ($runfetchData as $dataView) {
                    if (
                        ...
                        if (!empty($name) && !empty($dateF) && !empty($dateT) && !empty($price)) {
                            ...
                            $insertQ = "INSERT INTO `client_cart`(`login_id`, `fetch_cart_id`, `db_name`, `companyname`, `availablefrom`, `availableto`, `price`) VALUES ('$clientLoginId', '$this->reqId','$this->reqDb','$name','$dateF','$dateT','$price')";
                            $runinsertQ = mysqli_query($connection, $insertQ);
                            if($runinsertQ){
                                header('Location: /expedia/operations/cart.php?id=' . htmlspecialchars(trim($this->reqId)) . '&db=' . htmlspecialchars(trim($this->reqDb)) . '');
                            }else{
