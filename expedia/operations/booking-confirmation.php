                ...
                foreach ($runfetchData as $dataView) {
                    if (
                        ...
                        if (!empty($name) && !empty($dateF) && !empty($dateT) && !empty($price)) {
                            ...
                            $insertQ = "INSERT INTO `client_cart`(`login_id`, `fetch_cart_id`, `db_name`, `companyname`, `availablefrom`, `availableto`, `price`, `adult_count`, `chldren_count`) VALUES ('$clientLoginId', '$this->reqId','$this->reqDb','$name','$dateF','$dateT','$price','$adult_count','$children_count')";
